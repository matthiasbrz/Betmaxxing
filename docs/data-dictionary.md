...
| `VerifiedReceipt` / `VerifiedReceiptBatch` / `AuditResult` | type | la provenance d'un reçu et le résultat d'un audit. Depuis D-077 **aucun des trois n'est constructible** : chacun prend un jeton privé en premier argument positionnel, `trust()` et `require_verified()` sont supprimés, et le seul chemin est `audit_directory`, qui reçoit un répertoire déjà ouvert composant par composant et fait lui-même lecture, contrôle de schéma et HMAC contre le secret de l'installation. `qualification.evaluate` n'accepte qu'un `AuditResult` : ni liste, ni tuple, ni lot assemblé à la main. Un reçu admis est **gelé récursivement** — `FrozenMapping` implémente `Mapping` **sans hériter d'aucun conteneur mutable**, les séquences sont des `tuple`, et les quinze appels directs aux mutateurs de `dict` et de `list` lèvent ; l'audit scelle en plus une empreinte sha256 non clée par reçu, que `require_audited` recalcule. La v6, qui exposait un constructeur public sans contrôle et ne gelait qu'un niveau, laissait un corpus non signé atteindre la porte et une écriture imbriquée changer un verdict après vérification (D-076, corrigée par D-077) |
