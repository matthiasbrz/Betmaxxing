...
| `VerifiedReceipt` / `VerifiedReceiptBatch` | type | la provenance d'un reçu, frappée uniquement par `audit_receipts` après vérification de sa signature contre un secret **chargé** et jamais créé. `qualification.evaluate` n'accepte que ce type : passer des mappings bruts est refusé, et c'est ce qui garantit que l'évaluateur ne calcule aucun HMAC, ne lit ni environnement ni fichier et n'en écrit aucun (D-076) |
...
| `unresolved_attempt_intent_details` | `list[dict]` | ces intents, tels qu'écrits : identifiant, commande, portée, plafond contractuel, état. Aucune clé, aucune URL, aucun événement en clair, aucune cote, aucun payload (D-076) |
