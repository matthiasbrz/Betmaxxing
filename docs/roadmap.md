...
| `TheOddsApiProvider` | ⚠️ `IMPLEMENTED_UNVERIFIED` | Contrats locaux verts. 6 appels réels le 2026-08-07 (4 gratuits, 2 payants, 2 crédits) ont validé auth, endpoint payant et comptabilité du coût ; le **mapping des cotes reste non vérifié en réel** (`winamax_fr` absent des 2 événements testés). Critères de qualification préenregistrés en D-071, **corrigés par D-072 à D-076** (protocole v6, reçu v4, contrat conscient de la phase, atteinte fournisseur obligatoire pour toute preuve de réponse, secret strict de 64 hex, frontière du répertoire par descripteur, intent durable avant toute requête, commande à domaine fermé, six populations de coût, date d'effet `2026-08-11T04:50:40+00:00`) ; aucun satisfait, et les 6 appels du 2026-08-07 sont désormais de l'histoire non qualifiante |
...
| Promotion de l'adaptateur en `VERIFIED` | Une activation réussie est une preuve **limitée** (un endpoint, un bookmaker, une compétition, un événement, un marché, un instant). Les critères de promotion sont désormais **écrits, préenregistrés et opposables au runtime** — `PROVIDER_VALIDATION_PROTOCOL_VERSION = 6`, `PROVIDER_ADAPTER_EVIDENCE_VERSION = 1`, seuil littéral 900 s, date d'effet `2026-08-11T04:50:40+00:00`, reçu v4 exigé, contrat conscient de la phase, atteinte fournisseur établie exigée pour toute preuve de réponse, et `bookmaker_state = OBSERVED` obligatoire ; `docs/provider-validation-protocol.md`, D-071 à D-075 annotées, D-076 — et **aucun n'est encore satisfait** : la campagne n'a pas été exécutée. `activation status` dit précisément ce qui manque. La promotion elle-même reste une décision humaine ; la machine plafonne à `CRITERIA_MET_AWAITING_HUMAN_REVIEW` |
