...
| `TheOddsApiProvider` | ⚠️ `IMPLEMENTED_UNVERIFIED` | Contrats locaux verts. 6 appels réels le 2026-08-07 (4 gratuits, 2 payants, 2 crédits) ont validé auth, endpoint payant et comptabilité du coût ; le **mapping des cotes reste non vérifié en réel** (`winamax_fr` absent des 2 événements testés). Critères de qualification préenregistrés en D-071, **corrigés par D-072 à D-077**, modèle de menace de la provenance énoncé normativement par **D-078** (protocole v7, reçu v4, contrat conscient de la phase, atteinte fournisseur obligatoire pour toute preuve de réponse, secret strict de 64 hex à permissions vérifiées, frontière du répertoire par descripteur pour **tous** les lecteurs y compris `load_parent`, trois états de frontière dont `UNAVAILABLE` bloquant, provenance sans constructeur public — obtenue par l'audit HMAC, marqueur contre les erreurs d'usage ordinaires et non barrière contre du Python arbitraire exécuté dans le processus de confiance — reçu gelé récursivement, intent durable au contrat fermé résolu seulement par un reçu vérifié de même portée, quarantaine limitée aux reçus `.json`, commande à domaine fermé, six populations de coût, date d'effet `2026-08-11T14:20:00+00:00`) ; **protocole 7 clos sans qualification par D-082** après une découverte réelle unique le 2026-08-23 (`soccer_france_ligue_one` / `winamax_fr`, `COVERAGE_MISSING`, 0 événement admissible, 2 requêtes, 0 crédit) dont le reçu est conservé comme historique non qualifiant ; **protocole 8** préenregistre une campagne fermée et **opposable au runtime** — bookmaker `pinnacle`, compétitions `soccer_epl`, `soccer_spain_la_liga`, `tennis_atp_us_open`, `tennis_wta_us_open`, date d'effet `2026-08-25T00:00:00+00:00`, plafonds 4/6/2 comptés depuis les reçus vérifiés et refusés **avant réseau** par `campaign_preflight`, dépassement publié en `EVIDENCE_CONFLICT`, comptes jamais publiés avant d'être établis ; **registre des douze étapes fermé par D-083** — répartition `2 + 1` par famille, ordre canonique des événements `(coup d'envoi, identifiant en octets UTF-8)`, découverte trop maigre refusée à l'étape gratuite, commande / compétition / rang / parent opposés avant la clé fournisseur, étape suivante publiée en cinq champs `campaign_next_*` ou `null` ensemble ; **note de rectification D-083 du 2026-09-13** — les cinq champs sont effectivement émis par `status`, la position est reconnue dans les reçus (rang dans l'ordre signé de la découverte applicable **et** identité du reçu parent), `COMPLETE` exige les douze étapes reconnues et non les seuls totaux, et une contradiction établie du registre est un `EVIDENCE_CONFLICT` ; aucun critère satisfait, et les 6 appels du 2026-08-07 sont de l'histoire non qualifiante |
...
| Promotion de l'adaptateur en `VERIFIED` | Une activation réussie est une preuve **limitée** (un endpoint, un bookmaker, une compétition, un événement, un marché, un instant). Les critères de promotion sont désormais **écrits, préenregistrés et opposables au runtime** — `PROVIDER_VALIDATION_PROTOCOL_VERSION = 8`, `PROVIDER_ADAPTER_EVIDENCE_VERSION = 1`, seuil littéral 900 s, date d'effet `2026-08-25T00:00:00+00:00`, reçu v4 exigé, contrat conscient de la phase, atteinte fournisseur établie exigée pour toute preuve de réponse, et `bookmaker_state = OBSERVED` obligatoire ; `docs/provider-validation-protocol.md`, D-071 à D-076 annotées, D-077, D-078 (modèle de menace : l'authenticité vient du HMAC vérifié à l'ingestion, pas du type Python), D-082 (manifeste de campagne fermé et plafonds opposables avant réseau), D-083 (registre des douze étapes, ordre canonique des événements et refus du rang non prévu avant la clé fournisseur) — et **aucun n'est encore satisfait** : la campagne n'a pas été exécutée. `activation status` dit précisément ce qui manque. La promotion elle-même reste une décision humaine ; la machine plafonne à `CRITERIA_MET_AWAITING_HUMAN_REVIEW` |
| Exécution de la campagne de qualification | **Campagne v7 abandonnée** : une de ses quatre découvertes a été consommée sans événement, et les trois places restantes ne permettaient plus d'établir les deux compétitions exigées par famille (D-082). La campagne **v8** est préenregistrée sur `pinnacle` et quatre compétitions nommées, bornée à **12 invocations CLI, 16 requêtes HTTP dont 8 payantes, et 16 crédits contractuels maximum** — bornes désormais **comptées par la machine** et non plus seulement documentaires : un audit statique avait mesuré que neuf découvertes et un corpus conforme de neuf reçus publiaient des valeurs identiques dans tous les champs. Un échec consomme son invocation et abandonne la campagne ; recommencer exige un nouveau protocole. **D-083** ferme en outre l'ordre des douze appels et l'événement de chacun : il ne reste aucune décision à prendre entre deux invocations, et `status --json` dit laquelle serait la prochaine. Avec arrêts anticipés écrits. La v1 du protocole appelait « 12 requêtes » ce qui était un compte d'invocations ; corrigé en D-072. Un appel payant au coût non établi fait désormais échouer `COST_CONFORMITY` (D-073), et une tentative confirmée non envoyée est recensée à part, non bloquante et non qualifiante (D-075). Chaque appel exige une autorisation humaine distincte ; aucun n'a été lancé dans cette tranche |
