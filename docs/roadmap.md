...
| Promotion de l'adaptateur en `VERIFIED` | Une activation réussie est une preuve **limitée** (un endpoint, un bookmaker, une compétition, un événement, un marché, un instant). Les critères de promotion sont désormais **écrits, préenregistrés et opposables au runtime** — `PROVIDER_VALIDATION_PROTOCOL_VERSION = 4`, `PROVIDER_ADAPTER_EVIDENCE_VERSION = 1`, seuil littéral 900 s, date d'effet `2026-08-10T09:11:48+00:00`, reçu v4 exigé, contrat structurel strict et conscient de la phase, et `bookmaker_state = OBSERVED` obligatoire ; `docs/provider-validation-protocol.md`, D-071, D-072 et D-073 annotées, D-074 — et **aucun n'est encore satisfait** : la campagne n'a pas été exécutée. `activation status` dit précisément ce qui manque. La promotion elle-même reste une décision humaine ; la machine plafonne à `CRITERIA_MET_AWAITING_HUMAN_REVIEW` |
| Exécution de la campagne de qualification | Préparée en deux pistes (parser générique, couverture Winamax), bornée à **12 invocations CLI, 16 requêtes HTTP dont 8 payantes, et 16 crédits contractuels maximum**, avec arrêts anticipés écrits. La v1 du protocole appelait « 12 requêtes » ce qui était un compte d'invocations ; corrigé en D-072. Un appel payant au coût non établi fait désormais échouer `COST_CONFORMITY` (D-073), et `paid_calls_that_never_left` exige un `false` booléen certain (D-074). Chaque appel exige une autorisation humaine distincte ; aucun n'a été lancé dans cette tranche |
