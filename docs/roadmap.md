...
| Promotion de l'adaptateur en `VERIFIED` | Une activation réussie est une preuve **limitée** (un endpoint, un bookmaker, une compétition, un événement, un marché, un instant). Les critères de promotion sont désormais **écrits, préenregistrés et opposables au runtime** — `PROVIDER_VALIDATION_PROTOCOL_VERSION = 5`, `PROVIDER_ADAPTER_EVIDENCE_VERSION = 1`, seuil littéral 900 s, date d'effet `2026-08-10T14:00:37+00:00`, reçu v4 exigé, contrat conscient de la phase, atteinte fournisseur établie exigée pour toute preuve de réponse, et `bookmaker_state = OBSERVED` obligatoire ; `docs/provider-validation-protocol.md`, D-071 à D-074 annotées, D-075 — et **aucun n'est encore satisfait** : la campagne n'a pas été exécutée. `activation status` dit précisément ce qui manque. La promotion elle-même reste une décision humaine ; la machine plafonne à `CRITERIA_MET_AWAITING_HUMAN_REVIEW` |
